...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Kirish — Suhrobbek Qayumov" },
      { name: "description", content: "Shaxsiy kabinetga kiring." },
    ],
  }),
  component: AuthPage,
});
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-6 py-16">
      <div className="w-full max-w-md">
        <Link to="/" className="text-sm text-muted-foreground hover:text-foreground mb-8 inline-block">← Bosh sahifa</Link>
        <div className="border border-border rounded-2xl p-8 bg-card">
          <h1 className="text-3xl font-serif mb-2">{mode === "signin" ? "Kirish" : "Ro'yxatdan o'tish"}</h1>
          <p className="text-muted-foreground text-sm mb-6">Shaxsiy kabinet va admin paneliga kirish</p>
          ...
          <button type="button" onClick={() => setMode(mode === "signin" ? "signup" : "signin")} className="text-sm text-muted-foreground hover:text-foreground mt-4 w-full text-center">
